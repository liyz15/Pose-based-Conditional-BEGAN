...
def train(config):
    ...
    logger = create_logger(
        ...
        log_path='logs/train_300W_LP_{}.log'.format(time_now)
    )
    ...
    checkpoint_dir = 'checkpoints_300W_LP'
    if torch.cuda.is_available():
        z = z.cuda()
    for ep in range(config.epoch):
        for i, (real_batch, pose_batch) in enumerate(dataloader):
            ...
            # Assign the fix batch
            if fixed_real_batch is None:
                fixed_real_batch = torch.zeros_like(real_batch)
                fixed_real_batch.data = real_batch.clone()
                fixed_pose_batch = torch.zeros_like(pose_batch)
                fixed_pose_batch.data = pose_batch.clone()
                if torch.cuda.is_available():
                    fixed_real_batch = fixed_real_batch.cuda()
                    fixed_pose_batch = fixed_pose_batch.cuda()
            ...
            if step % config.verbose_steps == 0:
                logger.info(
                    'It: {}\treal_loss_D: {:.4f}\tfake_loss_D: {:.4f}\tloss_G: {:.4f}\tkt: {:.4f}\tmeasure: {:.4f}'.format(
                        step,
            ...
            if step % config.save_steps == 0:
                ...
                draw_debug_image(fixed_fake_batch_vary,
