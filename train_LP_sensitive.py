...
def train(config):
    ...
    logger = create_logger(
        ...
        log_path='logs/train_LP_sensitive_{}.log'.format(time_now)
    )
    ...
    checkpoint_dir = 'checkpoints_LP_sensitive'
    if torch.cuda.is_available():
        z = z.cuda()
    ...
    for ep in range(config.epoch):
        for i, (real_batch, pose_batch) in enumerate(dataloader):
            ...
            loss_weight = torch.norm(pose_batch, dim=1) + 1.0
            loss_G = torch.mean(torch.abs(fake_output_G - fake_batch_pose) * loss_weight)
