import torch
import torch.nn as nn
import torch.nn.functional as F


class Interpolate(nn.Module):
    """
    This exists only because nn.Upsample is deprecated and I want to use nn.Sequential
    """
    def __init__(self, size=None, scale_factor=None, mode='nearest'):
        super(Interpolate, self).__init__()
        self.interp = nn.functional.interpolate
        self.size = size
        self.scale_factor = scale_factor
        self.mode = mode

    def forward(self, x):
        x = self.interp(x, size=self.size, scale_factor=self.scale_factor, mode=self.mode)
        return x


class Generator(nn.Module):
    def __init__(self):
        super(Generator, self).__init__()
        self.encoder = nn.Sequential(
            nn.Conv2d(3, 128, kernel_size=3, stride=1, padding=1),    # 128 x 64 x 64
            nn.ELU(True),
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1),  # 128 x 64 x 64
            nn.ELU(True),
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1),  # 128 x 64 x 64
            nn.ELU(True),
            nn.Conv2d(128, 128, kernel_size=3, stride=2, padding=1),  # 128 x 32 x 32
            nn.ELU(True),
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1),  # 128 x 32 x 32
            nn.ELU(True),
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1),  # 128 x 32 x 32
            nn.ELU(True),
            nn.Conv2d(128, 128, kernel_size=3, stride=2, padding=1),  # 128 x 16 x 16
            nn.ELU(True),
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1),  # 128 x 16 x 16
            nn.ELU(True),
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1),  # 128 x 16 x 16
            nn.ELU(True),
            nn.Conv2d(128, 128, kernel_size=3, stride=2, padding=1),  # 128 x 8 x 8
            nn.ELU(True),
        )
        # Input shape: (N, 131)
        self.FC1 = nn.Linear(8192, 128)
        self.FC2 = nn.Linear(131, 8192)
        # Reshape to: (128 x 8 x 8)
        self.decoder = nn.Sequential(
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1),  # 128 x 8 x 8
            nn.ELU(True),
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1),  # 128 x 8 x 8
            nn.ELU(True),
            Interpolate(scale_factor=(2, 2), mode='nearest'),
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1),  # 128 x 16 x 16
            nn.ELU(True),
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1),  # 128 x 16 x 16
            nn.ELU(True),
            Interpolate(scale_factor=(2, 2), mode='nearest'),
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1),  # 128 x 32 x 32
            nn.ELU(True),
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1),  # 128 x 32 x 32
            nn.ELU(True),
            Interpolate(scale_factor=(2, 2), mode='nearest'),
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1),  # 128 x 64 x 64
            nn.ELU(True),
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1),  # 128 x 64 x 64
            nn.ELU(True),
            nn.Conv2d(128, 3, kernel_size=3, stride=1, padding=1),  # output_channel x 64 x 64
        )

    def forward(self, x):
        x, poses = x
        x = self.encoder(x)
        x = x.view(-1, 8192)
        x = self.FC1(x)
        x = torch.cat([x, poses], dim=1)
        x = self.FC2(x)
        x = x.view(-1, 128, 8, 8)
        x = self.decoder(x)
        return x


class Discriminator(nn.Module):
    def __init__(self):
        super(Discriminator, self).__init__()
        # Input shape: (N, 6, 64, 64)
        self.encoder = nn.Sequential(
            nn.Conv2d(6, 128, kernel_size=3, stride=1, padding=1),    # 128 x 64 x 64
            nn.ELU(True),
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1),  # 128 x 64 x 64
            nn.ELU(True),
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1),  # 128 x 64 x 64
            nn.ELU(True),
            nn.Conv2d(128, 128, kernel_size=3, stride=2, padding=1),  # 128 x 32 x 32
            nn.ELU(True),
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1),  # 128 x 32 x 32
            nn.ELU(True),
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1),  # 128 x 32 x 32
            nn.ELU(True),
            nn.Conv2d(128, 128, kernel_size=3, stride=2, padding=1),  # 128 x 16 x 16
            nn.ELU(True),
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1),  # 128 x 16 x 16
            nn.ELU(True),
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1),  # 128 x 16 x 16
            nn.ELU(True),
            nn.Conv2d(128, 128, kernel_size=3, stride=2, padding=1),  # 128 x 8 x 8
            nn.ELU(True),
        )
        # Reshape to: (N, 8192)
        self.FC1 = nn.Linear(8192, 128)
        self.FC2 = nn.Linear(128, 8192)
        self.decoder = nn.Sequential(
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1),  # 128 x 8 x 8
            nn.ELU(True),
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1),  # 128 x 8 x 8
            nn.ELU(True),
            Interpolate(scale_factor=(2, 2), mode='nearest'),
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1),  # 128 x 16 x 16
            nn.ELU(True),
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1),  # 128 x 16 x 16
            nn.ELU(True),
            Interpolate(scale_factor=(2, 2), mode='nearest'),
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1),  # 128 x 32 x 32
            nn.ELU(True),
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1),  # 128 x 32 x 32
            nn.ELU(True),
            Interpolate(scale_factor=(2, 2), mode='nearest'),
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1),  # 128 x 64 x 64
            nn.ELU(True),
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1),  # 128 x 64 x 64
            nn.ELU(True),
            nn.Conv2d(128, 6, kernel_size=3, stride=1, padding=1),  # output_channel x 64 x 64
        )

    def forward(self, x):
        x = self.encoder(x)
        x = x.view(-1, 8192)
        x = self.FC1(x)
        x = self.FC2(x)
        x = x.view(-1, 128, 8, 8)
        x = self.decoder(x)
        return x
